...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const ApplianceSchedulerScreen(),
    );
  }
}

class ApplianceSchedulerScreen extends StatefulWidget {
  static const String id = 'app';
  const ApplianceSchedulerScreen({Key? key});

  @override
  _ApplianceSchedulerScreenState createState() => _ApplianceSchedulerScreenState();
}

class _ApplianceSchedulerScreenState extends State<ApplianceSchedulerScreen> {
  final DatabaseReference _lightRef = FirebaseDatabase.instance.ref().child('Light');
  final DatabaseReference _fanRef = FirebaseDatabase.instance.ref().child('Light1');
  final DatabaseReference _acRef = FirebaseDatabase.instance.ref().child('AC');
  final DatabaseReference _tvRef = FirebaseDatabase.instance.ref().child('Fan');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Appliance Scheduler'),
      ),
      body: Center(
        child: Column(
          ...
          children: <Widget>[
            ...
            if (_selectedDateTime != null) ...[
              ...
              ElevatedButton(
                ...
                child: const Text('Schedule AC to Turn On'),
              ),
              ElevatedButton(
                onPressed: () {
                  _scheduleApplianceChange(false, _selectedDateTime!, _acRef);
                },
                child: const Text('Schedule AC to Turn Off'),
              ),
              ElevatedButton(
                onPressed: () {
